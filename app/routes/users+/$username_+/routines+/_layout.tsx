...
export async function loader({ params }: LoaderFunctionArgs) {
	...
	return json({ owner: { ...owner, routines } })
}

/*
if url matches routine detail (if it includes routine id)
...
export default function RoutinesRoute() {
	...
	const hasDetails = Boolean(params?.routineId)
						...
						<ul className="overflow-y-auto overflow-x-hidden pb-12">
							{isOwner ? (
								<li className="p-1 pr-0">
									...
									>
										<Icon name="plus">New Routine</Icon>
									</NavLink>
								</li>
							) : null}
							{data.owner.routines.map((routine) => (
								<li key={routine.id} className="p-1 pr-0">
									<NavLink
										to={routine.id}
										preventScrollReset
										prefetch="intent"
